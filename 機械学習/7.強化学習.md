...
1. 期待値が一番大きい店（水色の店）を選択し、実際の満足度を確認する
![Thompson Sampling](pic/64.png)

2. 1でえられた満足度（水色の四角）をもとにグラフを修正する
![Thompson Sampling2](pic/65.png)

3. 1～2を繰り返して学習する
![Thompson Sampling3](pic/66.png)

## Thompson Samplingアルゴリズム

![Thompson Sampling4](pic/68.png)
...
# Thompson Samplingアルゴリズムの実装
N = 10000 # データの数
d = 10 # 広告の数
ads_selected = [] # 選択された広告の番号を格納するリストを定義
numbers_of_rewards_1 = [0] * d # 各広告が報酬1を得た回数を格納するリストを定義
numbers_of_rewards_0 = [0] * d # 各広告が報酬0を得た回数を格納するリストを定義
total_reward = 0 # 総報酬を格納する変数を定義

for n in range(0, N): # データの数だけループを実行
    ad = 0 # 選択された広告の番号を格納する変数を初期化
    max_random = 0 # 最大値を格納する変数を初期化
    
    for i in range(0, d): # 広告の数だけループを実行
        random_beta = random.betavariate(numbers_of_rewards_1[i] + 1, numbers_of_rewards_0[i] + 1) # 広告iのランダムな値をベータ分布から生成（報酬1の回数+1と報酬0の回数+1をパラメータとして指定）
        
        if random_beta > max_random: # 広告iのランダムな値がこれまでの最大値より大きい場合
            max_random = random_beta # 最大値を更新
            ad = i # 広告iを選択
    ...
    if reward == 1: # 報酬が1の場合
        numbers_of_rewards_1[ad] += 1 # 広告adの報酬1の回数を更新
    else: # 報酬が0の場合
        numbers_of_rewards_0[ad] += 1 # 広告adの報酬0の回数を更新
...
# UCBとThompson Samplingの違い

![UCB vs Thompson Sampling](pic/67.png)