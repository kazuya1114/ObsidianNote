...
def inspect(results):
    ...
    rhs = [tuple(result[2][0][1])[0] for result in results] # ルールの右辺を抽出してリストに格納
    supports = [result[1] for result in results] # ルールのサポート値を抽出してリストに格納
    return list(zip(lhs, rhs, supports)) # 左辺、右辺、サポート値、信頼度、リフト値をタプルとしてまとめたリストを返す

resultsinDataFrame = pd.DataFrame(inspect(results), columns = ['Left Hand Side', 'Right Hand Side', 'Support']) # ルールの左辺、右辺、サポート値、信頼度、リフト値を列として持つデータフレームを作成
resultsinDataFrame = resultsinDataFrame.nlargest(n = 10, columns = 'Support') # サポート値が最も高い上位10件のルールを抽出してデータフレームに格納
