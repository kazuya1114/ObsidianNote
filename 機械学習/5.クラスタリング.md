...
1. 樹形図の作成
![階層クラスタリング4](pic/53.png)
※凝集型の場合、赤⇒水色⇒緑⇒橙色⇒黒の順にクラスタリング

2. クラスターの数を決定
![階層クラスタリング5](pic/54.png)
※右図の距離（赤い矢印線）が一番長い箇所に横線を引き、交差する線の数がクラスターの数
...
# 最適なクラスタ数を決定するための樹形図の作成
# sch.linkage：データ間の距離を計算して階層的クラスタリングを実行する関数
# sch.linkage(X, method = 'ward')：階層的クラスタリングのリンク関数を使用して、データXの階層的クラスタリングを実行（method = 'ward'はウォード法を指定）
# sch.dendrogram()：樹形図を描画する関数
dendrogram = sch.dendrogram(sch.linkage(X, method = 'ward')) # データXの階層的クラスタリングを実行し、樹形図を描画
plt.title('Dendrogram') # グラフのタイトルを設定
plt.xlabel('Customers') # x軸のラベルを設定
plt.ylabel('Euclidean distances') # y軸のラベルを設定
plt.show() # グラフを表示

# 階層的クラスタリングのモデルの学習
# n_clusters=5：クラスタ数を5に設定（上記で描画したグラフで最も効率が良かったクラスタ数を指定）
hc = AgglomerativeClustering(n_clusters = 5, affinity = 'euclidean', linkage = 'ward') # クラスタ数を5に設定してAgglomerativeClusteringクラスのインスタンスを作成（affinityは距離の計算方法、linkageはクラスタリングの方法を指定）
y_hc = hc.fit_predict(X) # 階層的クラスタリングモデルをデータXに適合させ、各データポイントのクラスタ割り当てを予測してy_hcに格納

# クラスタの可視化
# クラスタ0を赤色、クラスタ1を青色、クラスタ2を緑色、クラスタ3をシアン色、クラスタ4をマゼンタ色でプロット
# y_hc == 0, 0はクラスタ0に属するデータポイントを選択し、X[y_hc == 0, 0]はそのデータポイントのx座標（Age列）を、X[y_hc == 0, 1]はそのデータポイントのy座標（EstimatedSalary列）を表す
plt.scatter(X[y_hc == 0, 0], X[y_hc == 0, 1], s = 100, c = 'red', label = 'Cluster 1') # クラスタ0のデータポイントを赤色でプロット
plt.scatter(X[y_hc == 1, 0], X[y_hc == 1, 1], s = 100, c = 'blue', label = 'Cluster 2') # クラスタ1のデータポイントを青色でプロット
plt.scatter(X[y_hc == 2, 0], X[y_hc == 2, 1], s = 100, c = 'green', label = 'Cluster 3') # クラスタ2のデータポイントを緑色でプロット
plt.scatter(X[y_hc == 3, 0], X[y_hc == 3, 1], s = 100, c = 'cyan', label = 'Cluster 4') # クラスタ3のデータポイントをシアン色でプロット
plt.scatter(X[y_hc == 4, 0], X[y_hc == 4, 1], s = 100, c = 'magenta', label = 'Cluster 5') # クラスタ4のデータポイントをマゼンタ色でプロット
plt.title('Clusters of customers') # グラフのタイトルを設定
plt.xlabel('Age') # x軸のラベルを設定
plt.ylabel('Estimated Salary') # y軸のラベルを設定
plt.legend() # 凡例を表示
plt.show() # グラフを表示
```