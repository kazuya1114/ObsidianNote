...
# Kernel PCAの適用
#  KernelPCAクラスは、カーネル主成分分析を適用するためのクラスです。引数には、削減後の次元数とカーネル関数が指定されます。
#  n_components=2は、削減後の次元数を指定する引数です。これにより、特徴量が2次元に削減されます。削減後の次元数は、データセットの分散を最大限に保持するために選択されます。
#  kernel='rbf'は、カーネル関数としてRBF（Radial Basis Function）を使用することを指定する引数です。RBFカーネルは、非線形な特徴量空間を作成するために使用されます。これにより、線形分離できないデータセットでも、特徴量を削減して分類することができます。
kpca = KernelPCA(n_components=2, kernel='rbf')
X_train = kpca.fit_transform(X_train) # fit_transformメソッドは、Kernel PCAを適用して特徴量を削減するためのメソッドです。引数には、訓練セットの特徴量の配列が指定されます。これにより、訓練セットの特徴量が削減されます。
X_test = kpca.transform(X_test) # transformメソッドは、Kernel PCAを適用して特徴量を削減するためのメソッドです。引数には、テストセットの特徴量の配列が指定されます。これにより、テストセットの特徴量が削減されます。
