...
# 　remainder='passthrough'：指定しなかった列はそのまま残す
...
| 手法                      | 説明                    | 用途                        |
| ----------------------- | --------------------- | ------------------------- |
| **標準化（StandardScaler）** | 平均0、標準偏差1に変換（Z-score） | 勾配降下法、SVM、ロジ回帰、NN、K-means |
| **正規化（MinMaxScaler）**   | 値を0〜1に変換              | ディープラーニング、距離ベース           |
| **RobustScaler**        | 外れ値の影響を抑えてスケール調整      | 外れ値が多いデータ                 |
元のデータが標準正規分布に従っている場合は標準化をした方がいい。   
※`状況に応じてはフィーチャースケーリングは不要（例：単回帰分析）`
